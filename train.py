...
def build_everything(args: arg_util.Args):
    ...
    var: DDP = (DDP if dist.initialized() else NullDDP)(var_wo_ddp,
    ...
    print(f'[INIT][#para] ' + ', '.join([
        f'{k}={count_p(m)}' for k, m in (('VAE', vae_local), ('VAE.enc', vae_local.encoder),
    ...
    # build optimizer
    names, paras, para_groups = filter_params(var_wo_ddp,
                                              nowd_keys={
                                                  'cls_token',
                                                  'start_token',
                                                  'task_token',
    ...
    var_optim = AmpOptimizer(mixed_precision=args.fp16,
...
def main_training():
    ...
    for ep in range(start_ep, args.ep):
        ...
        L_mean, L_tail, acc_mean, acc_tail, grad_norm = stats['Lm'], stats['Lt'], stats[
            'Accm'], stats['Acct'], stats['tnm']
        ...
        if is_val_and_also_saving:
            ...
            best_val_acc_mean, best_val_acc_tail = max(best_val_acc_mean, val_acc_mean), max(
                best_val_acc_tail, val_acc_tail)
            AR_ep_loss.update(vL_mean=val_loss_mean,
            ...
            if dist.is_local_master():
                ...
                print(f'     [saving ckpt](*) finished!  @ {local_out_ckpt}',
                      flush=True,
                      clean=True)
            dist.barrier()
    ...
    args.remain_time, args.finish_time = '-', time.strftime("%Y-%m-%d %H:%M",
...
def train_one_ep(ep: int, is_first_ep: bool, start_it: int, args: arg_util.Args,
    ...
    for it, data in me.log_every(start_it, iters_train, ld_or_itrt, 1, header):
        # inp [-1, 1] (8. 3. 256. 256),
        # label tensor([835, 445, 594, 312, 701, 177, 666, 331])
        ...
        min_tlr, max_tlr, min_twd, max_twd = lr_wd_annealing(args.sche,
        ...
        if args.pg:  # default: args.pg == 0.0, means no progressive training, won't get into this
            if g_it <= wp_it:
                ...
                progress = min(max((g_it - wp_it) / (max_it * args.pg - wp_it), 0),
                               1)  # from 0 to 1
                prog_si = args.pg0 + round(
                    progress * delta)  # from args.pg0 to len(args.patch_nums)-1
        else:
            prog_si = -1
