...
class VAR(nn.Module):
    ...
    def __init__(
        ...
        self.uniform_prob = torch.full((1, num_classes),
        ...
        self.shared_ada_lin = nn.Sequential(nn.SiLU(
            inplace=False), SharedAdaLin(self.D, 6 * self.C)) if shared_aln else nn.Identity()
    ...
    def autoregressive_infer_cfg(
        ...
        next_token_map = sos.unsqueeze(1).expand(2 * B, self.first_l, -1) + self.pos_start.expand(
            2 * B, self.first_l, -1) + lvl_pos[:, :self.first_l]
    ...
    def forward(self, label_B: torch.LongTensor, x_BLCv_wo_first_l: torch.Tensor) -> torch.Tensor:  # returns logits_BLV
        ...
        :param x_BLCv_wo_first_l: teacher forcing input (B, self.L-self.first_l, self.Cvae) self.Cvae = 32
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            sos = cond_BD = self.class_emb(label_B)  # cond_BD:(1, 1024) label_B:(1)
            sos = sos.unsqueeze(1).expand(B, self.first_l, -1) + self.pos_start.expand(B, self.first_l, -1)

            if self.prog_si == 0: # self.prog_si = -1 False
                x_BLC = sos  
            else:
        ...
        AdaLNSelfAttn.forward
        for i, b in enumerate(self.blocks):  # Main
            x_BLC = b(x=x_BLC, cond_BD=cond_BD_or_gss, attn_bias=attn_bias)
            # x_BLC (1, 680, 1024)
            # cond_BD_or_gss (1, 1024)
            # attn_bias (1, 1, 680, 680)
    ...
    def init_weights(self, init_adaln=0.5, init_adaln_gamma=1e-5, init_head=0.02, init_std=0.02, conv_std_or_gain=0.02):
        ...
        for m in self.modules():
            ...
            # conv: VAR has no conv, only VQVAE has conv
                    ...
                    m, (nn.Conv1d, nn.Conv2d, nn.Conv3d, nn.ConvTranspose1d, nn.ConvTranspose2d, nn.ConvTranspose3d)):
